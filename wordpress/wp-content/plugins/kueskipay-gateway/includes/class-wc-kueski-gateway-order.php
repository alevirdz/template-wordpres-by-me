...
class Kueski_Gateway_Order
{
    ...
    protected function get_line_items_from_order($order, $ototal)
    {
        ...
        foreach ($order->get_items() as $cart_item_key => $values) {
            ...
            $item->description = Kueski_Gateway_Helper::remove_emojis($item->description);
            $item->description = Kueski_Gateway_Helper::clean_string($item->description);
            
            if (strlen($item->description) > 128) {
                $item->description = substr($item->description, 0, 124) . '...';
            }
            $item->description = trim( $item->description ) == '' ? $item->name : $item->description;
