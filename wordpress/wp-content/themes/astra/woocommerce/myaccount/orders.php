...
<?php if ( $has_orders ) : ?>
    ...
    if ( ! empty( $actions ) ) {
        foreach ( $actions as $key => $action ) :
            $icon_class = ''; // Inicializa una variable para el icono

            // Asignar iconos dependiendo del nombre de la acción
            switch ( $action['name'] ) {
                case 'View':
                    $icon_class = 'fas fa-eye'; // Icono de vista
                    break;
                case 'Cancel':
                    $icon_class = 'fas fa-times'; // Icono de cancelar
                    break;
                case 'Pay':
                    $icon_class = 'fas fa-credit-card'; // Icono de pagar
                    break;
            }
            ?>
            <a href="<?php echo esc_url( $action['url'] ); ?>" class="woocommerce-button button <?php echo sanitize_html_class( $key ); ?>" aria-label="<?php echo esc_attr( $action['name'] ); ?>">
                <i class="<?php echo esc_attr( $icon_class ); ?>"></i> <!-- Aquí se agrega el icono -->
